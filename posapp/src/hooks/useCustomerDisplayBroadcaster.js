...
const useCustomerDisplayBroadcaster = () => {
  ...
  const buildCartSnapshot = () => {
    ...
    return {
      customerName,
      items: cartItems.map(({ item_code, qty, rate, amount }) => ({ item_code, qty, rate, amount })),
      netTotal,
      taxRows,
      totalTax,
      discountAmount,
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [cartItems, customerName, discountOn, discountPercentage, taxTemplateRows, currencySymbol, currencyPrecision]);

  const sendPaymentStatus = (payload) => {
    if (channelRef.current) sendCustomerDisplayMessage(channelRef.current, "PAYMENT_STATUS", payload);
  };

  const sendInvoiceComplete = (payload) => {
    if (channelRef.current) sendCustomerDisplayMessage(channelRef.current, "INVOICE_COMPLETE", payload);
  };
